...
def get_event():
    events = [
        ...
        "liked_item",
        "reviewed_item",
        "entered_payment_method",
        "clicked_review",
        "clicked_item_description",
    ]
...
if __name__ == "__main__":
    ...
    y_region = detect_region()
    # print(f"The current region is set to {y_region}.")
    client = boto3.client("kinesis", region_name=y_region)
    ...
    while True:
        ...
        response = client.put_record(
            StreamName=stream_name, Data=encoded_data, PartitionKey="partitionkey"
        )

        if len(verbose) > 0:
            print("Message written to the Kinesis Data Stream")
